...
def get_data(rule_links, w, row):
    for link in rule_links:
        ...
        df = pd.DataFrame(
            columns=[title, aws_service, risk_level, rule_id, description, rationale, audit_goal, audit_using_console,
                     audit_using_cli, remediation_using_console, remediation_using_cli, link])
...
def main():
    ...
    soup = BeautifulSoup(source, 'html.parser')
    w = pd.ExcelWriter("aws_bp_rules.xlsx")
    row = 0;
    df = pd.DataFrame(
        columns=["Title", "AWS Service", "Risk", "ID", "Description", "Rationale", "Audit Goal", "Audit Console",
                 "Audit Cli", "Remediation Console", "Remediation Cli", "Link"])
    df.to_excel(w, startrow=row, index=False)
    w.save()
