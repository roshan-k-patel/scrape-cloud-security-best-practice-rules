...
def get_data(rule_links, filename):
    for link in rule_links:
        source = requests.get(link).text
        soup = BeautifulSoup(source, 'html.parser')

        try:
            aws_service = soup.find("div", {"class":"nav"}).text.strip()

            while '  ' in aws_service:
                aws_service = aws_service.replace('  ', '')

            aws_service = aws_service.split("\n")
            aws_service = ' '.join(aws_service)
            aws_service = aws_service.split("  ")
            aws_service = aws_service[2].strip()
        except:
            aws_service = ''
        ...
        try:
            audit_using_console = steps[1].text.replace(',','').replace("\n","")
        except:
            audit_using_console = ''

        try:
            audit_using_cli = steps[3].text.replace(',', '').replace("\n","")
        except:
            audit_using_cli = ''

        try:
            remediation_using_console = steps[5].text.replace(',', '').replace("\n","")
        except:
            remediation_using_console = ''

        try:
            remediation_using_cli = steps[7].text.replace(',', '').replace("\n","")
        except:
            remediation_using_cli = ''

        # format the relevant data into a csv friendly string, and then write it to the file
        csv_string = title + "," + aws_service + "," + risk_level + ',' + rule_id + ',' + description + ',' + rationale + ',' + audit_goal + ',' + audit_using_console + ',' + audit_using_cli + ',' + remediation_using_console + ',' + remediation_using_cli + "," + link
        print(csv_string)
...
def main():
    ...
    address = "https://www.cloudconformity.com/knowledge-base/aws/"
    source = requests.get(address).text
    soup = BeautifulSoup(source, 'html.parser')

    # name of file to write to and headers for csv
    filename = "aws_best_practice_rules.csv"
    headers = "Title, AWS Service, Risk, ID, Description, Rationale, Audit Goal, Audit Console, Audit Cli, Remediation Console, Remediation Cli, Link\n"
